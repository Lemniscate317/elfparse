...
public class Main {
    ...
    public static void main(String[] args) {
        byte[] bytes = Utils.readFile("so/libhello-jni.so");
...
//初始化AddSection中的变量
         ...
         */
        bytes = AddSection.addSectionHeader(bytes);
        bytes = AddSection.addNewSectionForFileEnd(bytes);
        bytes = AddSection.changeStrtabLen(bytes);
        bytes = AddSection.changeElfHeaderSectionCount(bytes);
        bytes = AddSection.changeProgramHeaderLoadInfo(bytes);

        Utils.saveFile("so/libhello-jnis.so", bytes);
    ...
    private static void parseSo(byte[] fileBytes) {
        System.out.println("++++++Elf header++++++");
        parseElfHeader(fileBytes, 0);
        ...
        parseProgramHeaderList(fileBytes, header_offset);
        ...
        praseSectionHeaderList(fileBytes, s_header_offset);
		...
		parseSymbolTableList(fileBytes, num_sym, offset_sym);
		type_32.printSymList();

        //读取字符串表信息(String Table)
        System.out.println();
        System.out.println("+++++++++++++++++++Symbol Table++++++++++++++++++");
        //这里需要注意的是：在Elf表中没有找到StringTable的数目，但是我们仔细观察Section中的Type=STRTAB段的信息，可以得到，这个字符串的大段的大小和偏移地址，但是我们这时候我们不知道小，所以就获取不到数目了
    ...
    private static void praseSectionHeaderList(byte[] header, int offset) {//这里得header应该是所有得sectionHeader
        int header_size = 40;//40 个字节 pdf 算一下相加
    ...
    private static void parseElfHeader(byte[] header, int offset) {
        if (header == null) {
